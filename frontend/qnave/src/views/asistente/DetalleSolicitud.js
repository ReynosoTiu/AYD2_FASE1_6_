...
const DetalleSolicitud = () => {
    ...
    useEffect(() => {
        const fetchSolicitud = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/asistant/getDriverPending/${id}`);
                if (!response.ok) {
    ...
    const handleProcesarSolicitud = async (idEstado) => {
        try {
            const response = await fetch(`http://localhost:8080/api/asistant/aproveRejectDriver`, {
                method: 'POST',
                headers: {
